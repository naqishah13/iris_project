...
auth_db = os.getenv("MONGO_AUTH_DB", "admin")  # Specify the authentication database

# URL encode username and password
encoded_username = quote_plus(username)
encoded_password = quote_plus(password)

# Construct the MongoDB URI with SSL/TLS settings
mongo_uri = (
    f"mongodb+srv://{encoded_username}:{encoded_password}@{cluster_url}/"
    f"{auth_db}?retryWrites=true&w=majority&tls=true"
)
...
try:
    client = MongoClient(
        ...
        tlsAllowInvalidCertificates=True  # Use this if you are facing issues with certificate verification
    )
